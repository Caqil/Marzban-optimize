...
def get_tls():
    ...
    try:
        return loop.run_until_complete(_get_tls())
    finally:
        loop.close()
...
async def _change_node_status(node_id: str, status: NodeStatus, message: str = None, version: str = None):
    try:
        ...
        await db.update_node_status(dbnode, status, message, version)
    except Exception:
        pass
...
def connect_node(node_id, config=None):
    ...
    async def _async_connect_node():
        ...
        try:
            ...
            await _change_node_status(node_id, NodeStatus.connecting)
            ...
            await _change_node_status(node_id, NodeStatus.connected, version=version)
            logger.info(f"Connected to \"{dbnode.name}\" node, xray run on v{version}")

        except Exception as e:
            await _change_node_status(node_id, NodeStatus.error, message=str(e))
    ...
    # Run async function
    asyncio.create_task(_async_connect_node())


@threaded_function
def restart_node(node_id, config=None):
    async def _async_restart_node():
        dbnode = await db.get_node_by_id(node_id)
    ...
    # Run async function
    asyncio.create_task(_async_restart_node())
