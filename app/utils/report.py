...
async def user_data_reset_by_next(user: UserResponse, user_admin: Admin = None) -> None:
    if NOTIFY_USER_DATA_USED_RESET:
        notify(UserDataResetByNext(username=user.username, action=Notification.Type.data_reset_by_next, user=user))


async def user_subscription_revoked(user: UserResponse, by: Admin, user_admin: Admin = None) -> None:
...
async def data_usage_percent_reached(
        percent: float, user: UserResponse, user_id: str, expire: Optional[int] = None, threshold: Optional[int] = None) -> None:
    if NOTIFY_IF_DATA_USAGE_PERCENT_REACHED:
        notify(ReachedUsagePercent(username=user.username, user=user, used_percent=percent))
        await db.create_notification_reminder(
...
async def expire_days_reached(days: int, user: UserResponse, user_id: str, expire: int, threshold=None) -> None:
...
async def login(username: str, password: str, client_ip: str, success: bool) -> None:
    if NOTIFY_LOGIN:
